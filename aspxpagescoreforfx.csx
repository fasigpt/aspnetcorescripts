...
var sos = new Extension(@"c:\\Windows\\Microsoft.NET\\Framework64\\v4.0.30319\\sos.dll");//this will load the sos.dll from the right runtime version from PATH C:\Program Files\dotnet\shared\Microsoft.NETCore.App
...
public void getThreaDIDforASPNetContext()
{
    ...
    string[] context = returnHeapforMethodTable("!name2ee mscorlib.dll System.Threading.Thread");
        ...
        if (!contextvalue.Contains("Address"))
        {
            if (contextvalue != string.Empty)
            {
                try
                {
                    ...
                    threadID = mex.Call("dq " + tempthreadcontext + "+40 L1");
                    ...
                    executionContext = d.Execute("dq  " + tempthreadcontext + "+10 L1");
        ...
        if (!contextvalue.Contains("Address"))
        {
            if (contextvalue != string.Empty)
            {
                try
                {
                    ...
                    requestProcessingStatus = mex.Call("dd " + _httpRequest._httpFrameContext + "+1b4 L1");
...
{
    if(objectstring.IndexOf("Address")>=0)
    {
        return (objectstring.Substring(objectstring.IndexOf("Address")).Split(linefeed, StringSplitOptions.RemoveEmptyEntries));
    }
...
public string returnHttpURLOfContext(string frameContext)
{
    ...
    string url = null;
    url = d.Execute("dq " + frameContext + "+b0 L1");
...
public cookieclass getAspNetCoreSession(string frameContext)
{
    ...
    cookie = mex.Call("dq " + frameContext + "+f8 L1");
...
//foreach(var item in _dbgThreadtoOSIDThreadDict)
//{
//    d.Output(item.Key);
//    d.Output("==");
//    d.Output(item.Value);
...
//    d.Output(item.Key);
//    d.Output("==");
//    d.Output(item.Value._finished);
//    d.Output("==");
//    d.Output(item.Value._pendingAction);
//    d.Output("==");
//    d.Output(item.Value._isPendingActionAsyncMethod);
//    d.Output("\n");
//}
